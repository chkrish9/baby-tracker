// Ported verbatim from web/src/lib/charts.ts — pure data functions, no
// rendering, so this is a direct copy with only import paths changed.
import { ML_PER_OZ, formatMinutes, formatMl, formatOz } from "@/lib/dates";
...
export interface FeedingLogLike {
  type: string;
  amount?: number | null;
...
export const FEED_EXTRA_COLUMNS = [
  {
    key: "bottle",
    label: "Bottle",
    format: (d: ChartDay) => ((d.extra?.bottleMl ?? 0) > 0 ? `${formatOz(d.extra!.bottleMl)} / ${formatMl(d.extra!.bottleMl)}` : "–"),
  },
  {
    key: "breastL",
    label: "Breast L",
    format: (d: ChartDay) => ((d.extra?.breastLeftMin ?? 0) > 0 ? formatMinutes(d.extra!.breastLeftMin) : "–"),
  },
  {
    key: "breastR",
    label: "Breast R",
    format: (d: ChartDay) => ((d.extra?.breastRightMin ?? 0) > 0 ? formatMinutes(d.extra!.breastRightMin) : "–"),
  },
  {
    key: "breastTotal",
    label: "Breast L+R",
