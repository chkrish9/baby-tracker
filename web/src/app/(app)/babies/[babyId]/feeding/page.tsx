...
export default function LogsPage({ params }: { params: Promise<{ babyId: string }> }) {
  ...
  async function handleDeleteFeed(logId: string) {
    const res = await apiFetch(`/api/babies/${babyId}/feeding/${logId}`, { method: "DELETE" });
  ...
  async function handleDeleteDiaper(logId: string) {
    const res = await apiFetch(`/api/babies/${babyId}/diapers/${logId}`, { method: "DELETE" });
  ...
  async function handleSaveDiaperFlags(logId: string, appointmentIds: string[]) {
    const res = await apiFetch(`/api/babies/${babyId}/diapers/${logId}/appointments`, {
      method: "PUT",
  ...
  async function handleLogFeed(e: React.FormEvent) {
    ...
    const res = await apiFetch(url, { method: editingFeedId ? "PATCH" : "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify(body) });
  ...
  async function handleLogDiaper(e: React.FormEvent) {
    ...
    const res = await apiFetch(url, { method: editingDiaperId ? "PATCH" : "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify(body) });
