...
export default function DoctorVisitPage({ params }: { params: Promise<{ babyId: string }> }) {
  ...
  async function handleSaveAppt(e: React.FormEvent) {
    ...
    const res = await apiFetch(url, { method: editingApptId ? "PATCH" : "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify(body) });
  ...
  async function handleDeleteAppt(id: string, e: React.MouseEvent) {
    e.preventDefault();
    e.stopPropagation();
    const res = await apiFetch(`/api/babies/${babyId}/appointments/${id}`, { method: "DELETE" });
