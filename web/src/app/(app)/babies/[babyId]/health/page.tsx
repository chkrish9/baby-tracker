...
export default function HealthPage({ params }: { params: Promise<{ babyId: string }> }) {
  ...
  async function handleSaveVacc(e: React.FormEvent) {
    ...
    const res = await apiFetch(url, { method: editingVaccId ? "PATCH" : "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify(body) });
    setVaccSaving(false);
  ...
  async function handleDeleteVacc(id: string) {
    const res = await apiFetch(`/api/babies/${babyId}/vaccinations/${id}`, { method: "DELETE" });
  ...
  async function handleSaveGrowth(e: React.FormEvent) {
    ...
    const res = await apiFetch(url, { method: editingGrowthId ? "PATCH" : "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify(body) });
  ...
  async function handleDeleteGrowth(id: string, growthType: GrowthType) {
    const res = await apiFetch(`/api/babies/${babyId}/growth/${id}`, { method: "DELETE" });
  ...
  async function handleSaveOther(e: React.FormEvent) {
    ...
    const res = await apiFetch(url, { method: editingOtherId ? "PATCH" : "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify(body) });
  ...
  async function handleDeleteOther(id: string) {
    const res = await apiFetch(`/api/babies/${babyId}/health-records/${id}`, { method: "DELETE" });
